...
pytestmark = [
    ...
    pytest.mark.skipif(
        not os.getenv("RUN_LLM_INTEGRATION_TESTS"), reason="Set RUN_LLM_INTEGRATION_TESTS=1 to run"
    ),
]
...
class TestChatRealLLM:
    ...
    async def test_chat_send_with_real_openai(self, client: AsyncClient, auth_headers: dict):
    ...
    async def test_chat_send_with_real_gemini(self, client: AsyncClient, auth_headers: dict):
