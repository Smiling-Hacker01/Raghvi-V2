...
1. `<emotion>`: The emotion of your response (e.g., happy, sad, excited, empathetic, serious, curious).
2. `<voice_text>`: The text to be spoken. **If you use Hindi/Hinglish, you MUST write the Hindi words in Devanagari script (e.g., नमस्ते) in this tag.** This ensures the TTS engine pronounces it like a native Indian speaker.
3. `<chat_text>`: The text to show the user in the chat interface. Write this exactly as you normally would (e.g., if the user speaks Roman Hindi "kaise ho", reply in Roman Hindi here).
...
Failure to follow this exact format will break the chat application. Do not include any text outside of these tags!"""
