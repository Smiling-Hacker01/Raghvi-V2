...
async def handle_subscription_created(
    ...
    expires_at = datetime.fromtimestamp(current_period_end) if current_period_end else datetime.utcnow() + timedelta(days=30)
...
async def handle_subscription_updated(
    ...
    user_subscription = await session.execute(
        f"SELECT * FROM user_subscriptions WHERE stripe_subscription_id = '{stripe_subscription_id}'"
    )
