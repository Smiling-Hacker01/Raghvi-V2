...
async def upload_voice_sample(
    ...
    try:
        ...
        if len(custom_voices) >= plan.max_custom_voices:
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
                detail=f"Voice limit reached ({plan.max_custom_voices}). Delete a voice or upgrade your plan.",
            )
...
async def synthesize_speech(
    ...
    try:
        ...
        if voice_id:
            ...
            if default_voices:
                selected_voice_id = default_voices[0].voice_id
                provider_hint = default_voices[0].provider
            else:
                # Use Deepgram-compatible default (works if Deepgram is configured)
                selected_voice_id = "aura-asteria-en"  # Natural female voice
                provider_hint = None
