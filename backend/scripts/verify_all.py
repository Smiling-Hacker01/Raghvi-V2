...
async def verify_all():
            ...
            if len(plans_list) >= 4:
                ...
                for plan_id in expected:
                    if plan_id in plan_ids:
                        ...
                        print_check(
                            f"  - {plan.name}: ${plan.price_usd} ({plan.max_custom_voices} voices)"
                        )
                    else:
    ...
    try:
        ...
        config_keys = [
            ...
            "elevenlabs_api_key",
            "voice_sample_rate",
            "voice_clone_max_file_size",
            "s3_bucket",
            "redis_voice_cache_ttl",
        ]
    ...
    async with AsyncSessionLocal() as session:
        try:
            ...
            await SubscriptionService.get_subscription_stats("test_user", session)
